/**
 * Generates a kernel matrix according to 
 * the user-specified graph-based kernel 
 * method over set of vertices of interest 
 * in a directed graph.
 *
...
void print_help()  {
    ...
    cout << "  -t TYPE    Kernel type (0-Cumulative Random Walk, 1-Random Walk, 2-Standard Digraphlet, 3-Label Substitutions Digraphlet, 4-Edge Indels Digraphlet, 5-Edit Distance Digraphlet).\n";
    ...
    cout << "  -g G_FILE  Inout directed graph file.\n";
    ...
    cout << "  -M LABMIS  Fraction of nodes in the n-digraphlet allowed to have vertex label mismatches. (Needed for Label Substitutions and Edit Distance Kernels)\n";
    cout << "             For example, M=0.51 allows 1-vertex label mismatch for n=2 and 3 digraphlets, whereas M=0.34 allows 1-vertex label mismatch for 3-digraphlets only.\n";
    cout << "             Defaults to 0.0 (i.e. no vertex label mismatches allowed).\n\n";

    cout << "  -E EDGMIS  Total number of edge insertions and/or deletions allowed between digraphlets. (Needed for Edge Indels Kernel and Edit Distance Kernels)\n";
    cout << "             Defaults to 0.\n\n";
...
int main(int argc, char* argv[])  {
    ...
    // Label Substitutions Digraphlet Kernel Parameters
    float mismatches(0.0);
    string alphabet; 
    string root_alphabet;
    string sim_matrix_file;

    // Edge Indels Digraphlet Kernel Parameter
    unsigned edgmis(0);
    ...
    for (int i=1; i<argc && (argv[i])[0] == '-'; i++)  {
        switch ((argv[i])[1])  {
            ...
            case 'g': i++; dg_file=argv[i]; break;
            case 'l': i++; nl_file=argv[i]; break;
    ...
    if (examples.size() < 2)  {
        cerr << "ERROR: Too few graphs." << endl << endl; print_help(); exit(1);
    }

    if (normalize)  gk.set_normalize();
    if (verbose)  gk.set_verbose();

    gk.read_graphs(nl_file, dg_file, examples);
