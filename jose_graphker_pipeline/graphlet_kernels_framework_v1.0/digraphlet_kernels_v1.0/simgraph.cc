...
SimpleGraph SimpleGraph::read_digraph(const char *nlabels_file, const char *digraph_file)  {
    ifstream lin(nlabels_file, ios::in);
    ifstream gin(digraph_file, ios::in);

    if (lin.fail())  {
        cerr << "ERROR: Vertex labels file " << nlabels_file << " could not be opened." << endl; exit(1);
    }

    if (gin.fail())  {
        cerr << "ERROR: Directed graph file " << digraph_file << " could not be opened." << endl; exit(1);
    }
    ...
    while(getline(gin, line))  {
        ...
        if (i >= g.adj.size())  {
            cerr << "ERROR: Node index " << i << " >= graph size " << g.adj.size() << " in graph file " << digraph_file << "." << endl; exit(1);
        }
        ...
        for (unsigned j=1; j<tokens.size(); j++)  {
            ...
            if (k >= g.adj.size())  {
                cerr << "ERROR: Node index " << k << " >= graph size " << g.adj.size() << " in graph file " << digraph_file << "." << endl; exit(1);
            }
